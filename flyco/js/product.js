...
var detailIMG = detailImg.children[0];
for(var i = 0; i < imgs.length; i++){
    ...
    imgs[i].onclick = function(){
        ...
        dt.appendChild(img);
        dt.appendChild(div);
        magnifying = document.querySelector(".magnifying");
...
//放大镜在往下，往右移动，有时到达不了最下，最右，存在空隙，待解决
function magnify(){
    firstImg.onmouseenter = function(e){
        magnifying.style.display = "block";
        //规定放大镜刚出现的左右位置
        e.offsetX <= 74 ? magnifying.style.left = 0 : magnifying.style.left = 226 + "px";
        //规定放大镜刚出现的上下位置
        e.offsetY <= 64 ? magnifying.style.top = 0 : magnifying.style.top = 239 + "px";
        //放大镜显示同时，商品大图也显示
    ...
    document.onmousemove = function(e){
        ...
        : e.pageX <= 351 ? magnifying.style.left = e.pageX - 127 + "px" : null ;
        //规定放大镜上下移动界线
        e.pageY <= 148 ? magnifying.style.top = 0 
        : e.pageY <= 386 ?  magnifying.style.top = e.pageY - 147 + "px" : null;
        //商品详情图片也显示
        //规定大图在窗口的左右移动界线
        e.pageX <= 127 ? detailIMG.style.left = 0 
        : e.pageX <= 351 ? detailIMG.style.left =  - (e.pageX - 127) * 2 + "px" : null ;
        //规定大图在窗口的上下移动界线
        e.pageY <= 148 ? detailIMG.style.top = 0 
        : e.pageY <= 386 ? detailIMG.style.top = - (e.pageY - 147) * 2 + "px" : null ;
    }
    firstImg.onmouseleave = function(){
        magnifying.style.display = "none";
        detailImg.style.display = "none";
    }
    magnifying.onmouseenter = function(){
